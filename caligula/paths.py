"""Canonical paths and caching manager for Caligula."""
...
def cache_path(category: str, key: str, ext: str = "parquet") -> Path:
    """Returns a deterministic path for cached files."""
    p = RAW_DIR / category
...
def read_cache(category: str, key: str, ext: str = "parquet"):
    """Reads a cached file if it exists."""
...
def write_cache(obj, category: str, key: str, ext: str = "parquet"):
    """Writes an object to the local cache."""
    p = cache_path(category, key, ext)
    if ext == "parquet":
        obj.to_parquet(p)
    elif ext == "json":
